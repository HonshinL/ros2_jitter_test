...
class JitterAnalyzer(Node):
    ...
    def plot_results(self):
        self.get_logger().info(f'进入plot_results函数，interval_count={self.interval_count}')
        ...
        # --- 新增：解决中文显示问题 ---
        matplotlib.rcParams['font.sans-serif'] = ['WenQuanYi Micro Hei']  # 使用微 Hei 字体
        ...
        self.get_logger().info(f'有效数据长度: {len(data)}')
        ...
        # 检查是否需要退出
        if self.should_exit:
            self.get_logger().info('程序将退出，不生成图表')
            return

        try:
            # 绘图
            self.get_logger().info('开始创建图表')
            ...
            self.get_logger().info(f'正在保存图片: {filename}')
            plt.savefig(filename, dpi=300, bbox_inches='tight')
            print(f"\n图表已保存为: {filename}")
            
            # 关闭figure，释放资源
            plt.close(fig)
            self.get_logger().info('图片保存完成')
            
        except Exception as e:
            self.get_logger().error(f'生成图表或保存图片时发生错误: {e}')
            print(f"\n生成图表或保存图片时发生错误: {e}")
...
def main():
    ...
    try:
        ...
        # 在主线程中调用绘图函数
        node.get_logger().info(f'准备调用plot_results函数，interval_count={node.interval_count}')
        if not node.should_exit and not node.has_plotted and node.interval_count > 0:
            node.get_logger().info('调用plot_results函数')
            node.plot_results()
            node.has_plotted = True
        elif node.interval_count == 0:
            node.get_logger().warning('没有采集到数据，跳过绘图')
            print("\n警告: 没有采集到数据，跳过绘图")
        else:
            node.get_logger().info('跳过绘图')
